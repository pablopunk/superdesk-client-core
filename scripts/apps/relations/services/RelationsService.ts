...
import {isPublished, isFetched, isIngested} from 'apps/archive/utils';
...
export function RelationsService(api, $q) {
    this.getRelatedKeys = function(item: IArticle, fieldId: string) {
        return Object.keys(item.associations || {})
            .filter(
                (key) =>
                    key.startsWith(fieldId) && item.associations[key] != null,
            )
            .sort(
                (a, b) =>
                    item.associations[a].order - item.associations[b].order,
            );
    };
    ...
    this.itemHasAllowedStatus = function(item: IArticle, field: IArticleField) {
        ...
        if (
            ALLOWED_WORKFLOWS.ingested !== true &&
            (isFetched(item) || isIngested(item))
        ) {
            return false;
        }
